...
41 suites: 40 native PowerShell suites (`tests/*.Tests.ps1`, including the shim below, 5,958 lines) and
...
PowerShell-child CI harness below can invoke it too - it is still counted among the 40, because it is
...
Pester. Each real suite defines its own `Assert` and prints `<name>: ALL PASS`. 1,145 `Assert` call sites
