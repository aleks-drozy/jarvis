...
35 suites: 34 native PowerShell suites (`tests/*.Tests.ps1`, including the shim below, 4,788 lines) and
...
PowerShell-child CI harness below can invoke it too - it is still counted among the 34, because it is
...
Pester. Each real suite defines its own `Assert` and prints `<name>: ALL PASS`. 951 `Assert` call sites
